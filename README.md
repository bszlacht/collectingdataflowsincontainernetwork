...
1. Start `minikube` cluster using below configuration and mount an *ebpf* filesystem:
...
2. Deploy *Prometheus* to collect network stats and *Grafana* for visualisation:
...
4. Deploy *Hubble* to observe networks flows:

```bash
kubectl apply -f https://raw.githubusercontent.com/cilium/cilium/v1.9/install/kubernetes/quick-hubble-install.yaml
```

5. Deploy *Inspektor Gadget*:

```bash
kubectl gadget deploy
```

6. Deploy an exemplary application (*Online Boutique*):
...
1. Observe network statistcs using *Grafana*:
...
2. Observe network flows using *Hubble UI*:
...
and later analyze exported flows with *Hubble CLI*:
